...
export const ServiceCard = ({
    ...
    return (
        <div className={`h-full ${wrapperClassName}`}>
            <div className={`service-card h-full ${cardBg} border border-[#07F8F2]/30 p-8 lg:p-5 xl:p-6 2xl:p-8 3xl:p-12 rounded-[1.5rem] flex flex-col gap-4 lg:gap-3 xl:gap-4 2xl:gap-5 3xl:gap-8 relative overflow-hidden ${hoverBg} transition-[background-color,border-color,box-shadow] duration-500 shadow-2xl group hover:border-[#07F8F2]/60 hover:shadow-[0_0_60px_rgba(7,248,242,0.1)] will-change-transform`}>
                ...
                <div className="relative z-10 flex flex-col gap-4 lg:gap-3 xl:gap-4 2xl:gap-5 3xl:gap-8 h-full">
                    <h3 className="text-white text-2xl lg:text-lg xl:text-xl 2xl:text-2xl 3xl:text-5xl font-bold leading-[1.1]">
                        {titleBase}
                        {titleLineBreak && <br />}
                        <span className={titleAccentColor}>{titleAccent}</span>
                    </h3>
                    <p className="text-gray-100 text-sm lg:text-[14px] xl:text-[15px] 2xl:text-base 3xl:text-xl leading-relaxed flex-1 font-medium">
                        {description}
                    </p>
                    <CTAButton 
                        fullWidth 
