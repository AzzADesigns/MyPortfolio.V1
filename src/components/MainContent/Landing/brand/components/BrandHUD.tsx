...
export const BrandHUD = ({ hoveredWord }: BrandHUDProps) => {
    const getRingStyles = (type: 'outer' | 'inner') => {
        ...
        if (type === 'outer') {
            switch (hoveredWord) {
                ...
                case 'personalizados': return `${base} inset-[-10%] border-[2px] border-[#89EA2B]/60 blur-[10px] animate-morph`;
                case 'fluidas': return `${base} inset-[-15%] rounded-full border-0 bg-[#07F8F2]/10 blur-[120px] scale-125 opacity-60`;
                case 'marca': return `${base} inset-[-10%] border-[1px] border-[#89EA2B]/40 blur-0 opacity-100 animate-master [animation-duration:2s]`;
                default: return `${base} inset-0 border-[40px] md:border-[80px] border-[#89EA2B]/10 blur-[50px] md:blur-[90px] animate-pulse-slow`;
            }
        }
        ...
        if (type === 'inner') {
            switch (hoveredWord) {
                ...
                case 'personalizados': return `${base} inset-[15%] border-[1px] border-[#07F8F2]/50 blur-[5px] animate-morph [animation-delay:1000ms]`;
                case 'fluidas': return `${base} inset-[20%] rounded-full border-[1px] border-[#07F8F2]/80 blur-[2px] scale-90 opacity-90 shadow-[0_0_30px_rgba(7,248,242,0.5)]`;
                ...
                default: return `${base} inset-[15%] md:inset-[18%] border-[30px] md:border-[60px] border-[#07F8F2]/15 blur-[40px] md:blur-[70px] animate-pulse-medium`;
            }
        }
        return ""; 
    };
    ...
    return (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] md:w-[650px] md:h-[650px] lg:w-[850px] lg:h-[850px] 3xl:w-[1100px] 3xl:h-[1100px] pointer-events-none">
            ...
            <div className={`absolute inset-[-20%] bg-[#07F8F2]/5 rounded-full blur-[150px] transition-all duration-1000 scale-100 animate-pulse-slow ${hoveredWord === 'marca' ? 'opacity-0' : 'opacity-40'}`}></div>
        </div>
    );
};
